  ...
  commands: {
    ...
    tests: {
      ...
      'add-question': {
        ...
        type_specific: {
          multiple_choice: ['--choices <items...> (min 2)', '--allow-multiple', '--randomize-choices', '--asset-id <id>'],
          likert: ['--scale-type <scale>', '--custom-choices <items...> (when scale-type=custom)', '--asset-id <id>'],
          nps: ['--asset-id <id>'],
          free_response: ['--asset-id <id>', '--site-link <url>'],
          ranking: ['--choices <items...> (min 3)', '--asset-id <id>'],
          preference: ['--variations <json> (min 2; the image-carrying form)', '--choices <items...> (min 2, legacy alias; names only, no images)'],
          matrix: ['--choices <items...> (min 1, row labels)', '--categories <items...> (min 2, column labels)', '--asset-id <id>'],
